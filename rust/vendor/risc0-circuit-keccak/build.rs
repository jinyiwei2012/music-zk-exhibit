...
fn generate_zkr_table() {
    ...
    for po2 in 14..=18 {
        ...
        // Windows patch:canonicalize 在 Windows 返回 `\\?\C:\...`,反斜杠在
        // include_bytes! 字符串里是非法转义。统一转成正斜杠并去掉 \\?\ 前缀。
        let display_path = zkr_path.to_string_lossy().replace('\\', "/");
        ...
        writeln!(
            &mut output,
            "(include_bytes!(\"{display_path}\"), {size}),",
        )
        .unwrap();
    }
