             ...
             const bool is_intt, const fr_t d_domain_size_inverse)
{
    ...
    if (intermediate_mul == 2) {
        unsigned int rev_pos = bit_rev((unsigned int)thread_ntt_pos, stage);
        fr_t t = d_inner_twiddles[rev_pos];

        t *= r1;
        r1 = r0 - t;
        r0 = r0 + t;

        rev_idx |= rev_pos << iterations;
    } else {
        fr_t t = r1;
        r1 = r0 - t;
        r0 = r0 + t;
...
public:
    ...
    void step(int iterations)
    {
        ...
        #define NTT_ARGUMENTS d_inout, lg_domain_size, stage, iterations, \
                ...
                ntt_parameters.root19_twiddles, \
