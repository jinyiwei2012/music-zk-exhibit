...
template<class fr_t> __launch_bounds__(512) __global__
void generate_twiddles(fr_t* d_twiddles, unsigned int nbits, const fr_t root)
{
    unsigned int row_size = blockDim.x * gridDim.x;
    assert((row_size & (row_size-1)) == 0);

    unsigned int row_bits = 31 - __clz(row_size);
    assert(row_bits <= nbits);

    unsigned int tid = threadIdx.x + blockDim.x * blockIdx.x;

    fr_t twiddle = root^bit_rev(tid, nbits);

    d_twiddles[tid] = twiddle;

    nbits -= row_bits;
    for (unsigned int i = 1; i < (1<<nbits); i++) {
        twiddle *= root;
        d_twiddles[(bit_rev(i, nbits) << row_bits) + tid] = twiddle;
    }
}
...
public:
        ...
        : gpu(select_gpu(id)), inverse(_inverse)
    {
        ...
        generate_twiddles<<<1, 512, 0, gpu[0]>>>(inner_twiddles, 9, roots[10]);
        ...
        root19_twiddles = reinterpret_cast<decltype(root19_twiddles)>
	                  (gpu[0].Dmalloc(sizeof(fr_t) << 18));
        generate_twiddles<<<16, 512, 0, gpu[0]>>>(root19_twiddles, 18, roots[19]);
        CUDA_OK(cudaGetLastError());

#if 0 //ndef __CUDA_ARCH__
        fr_t x, y;

        gpu[0].DtoH(&x, &inner_twiddles[63], 1);
        gpu[0].DtoH(&y, &root18_twiddles[63], 1);
