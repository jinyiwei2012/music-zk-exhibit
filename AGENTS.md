...
- [x] CI(SPEC §17.5):`.github/workflows/ci.yml` **首次 push 已跑绿**——ubuntu 干净环境双构建 guest、R0BF SHA-256 两次一致(⇒ Image ID 一致);windows 纯 CPU 构建 verifier + 全量 pytest + 入库真实收据复验。**注意:guest ELF 经 file!() 嵌入绝对构建路径 → Image ID 路径敏感,跨机器复现冻结值不可行,见 OPEN-QUESTIONS**
...
- [x] README 状态更新(Image ID 计算命令 + Phase 5 完成状态)

**门禁**:PRD §13.2、§13.3 全过。
**实测(2026-09-01)**:§17.4 三件套全过(Rust 52 + Python 106 测试绿);CI 双 job 全绿(guest 双构建一致门禁 + windows verifier/pytest/真实收据复验)。
...
**第一步(现在做)**:Phase 5 = SPEC M4 已收尾(§17.4 隐私三件套、§17.5 CI 双 job 全绿、§18 性能报告全量、README 定稿)。下一步若继续:按 docs/PLAN.md 的"展望"评估——60s 负载 prove≤30min/receipt≤10MB/verify≤10s 目标差距的跟进路径(succinct receipt、segment 调参、remap-path-prefix 修复 Image ID 路径敏感并升 protocol_id),或进入展示/发布环节。
